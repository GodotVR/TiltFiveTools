...
func _on_pointer_event(event : T5ToolsPointerEvent) -> void:
	...
	if index < 0 or type == T5ToolsPointerEvent.Type.ENTERED:
	...
	match type:
		...
		T5ToolsPointerEvent.Type.RELEASED:
			_presses.erase(pointer)
			pressed = false
	...
	match type:
		...
		T5ToolsPointerEvent.Type.MOVED:
	...
	if not _presses.has(_mouse):
		if type == T5ToolsPointerEvent.Type.PRESSED and pointer is T5ToolsPointer:
			# Switch to pressed laser-pointer
			_mouse = pointer
		elif type == T5ToolsPointerEvent.Type.EXITED and pointer == _mouse:
